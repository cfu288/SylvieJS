...
describe("testing destructured serialization/deserialization", function () {
  ...
  it("verify individual partitioning works correctly", function () {
    ...
    let data = ddb.deserializeCollection(result, {
      partitioned: true,
      delimited: false,
    });
    ...
    data = ddb.deserializeCollection(result, {
      partitioned: true,
      delimited: true,
    });
...
describe("testing adapter functionality", function () {
  it("verify basic memory adapter functionality works", function (done) {
    ...
    const ddb = new loki("test.db", { adapter: memAdapter });
    ...
    ddb.saveDatabase(function (err) {
      ...
      const cdb = new loki("test.db", { adapter: memAdapter });
  ...
  it("verify partioning adapter works", function (done) {
    ...
    const adapter = new loki.LokiPartitioningAdapter(mem);

    const db = new loki("sandbox.db", { adapter: adapter });
    ...
    db.saveDatabase(function (err) {
      ...
      db.saveDatabase(function (err) {
        ...
        const db2 = new loki("sandbox.db", { adapter: adapter });
  ...
  it("verify partioning adapter with paging mode enabled works", function (done) {
    ...
    db.saveDatabase(function (err) {
      ...
      db.saveDatabase(function (err) {
        ...
        // ok now lets load from it
        let db2 = new loki("sandbox.db", { adapter: adapter });
        db2.loadDatabase();
  ...
  it("verify reference adapters get db reference which is copy and serializable-safe", function (done) {
    ...
    const db = new loki("rma test", { adapter: adapter });
    ...
    const db2 = new loki("other name", { adapter: adapter });
...
describe("async adapter tests", function () {
  it("verify throttled async drain works", function (done) {
    ...
    const db = new loki("sandbox.db", { adapter: mem, throttledSaves: true });

    // Add a collection to the database
    const items = db.addCollection("items");
    const mjol = items.insert({
      name: "mjolnir",
      owner: "thor",
      maker: "dwarves",
    });
    const gun = items.insert({
      name: "gungnir",
      owner: "odin",
      maker: "elves",
    });
    ...
    db.throttledSaveDrain(function () {
      ...
      const db2 = new loki("sandbox.db", { adapter: mem });
  ...
  it("verify throttledSaveDrain with duration timeout works", function (done) {
    ...
    const db = new loki("sandbox.db", { adapter: mem });
...
describe("verify serializereplacer", function () {
  it("verify verbose console is replaced", function () {
    const sdb = new loki("test.db", { verbose: true });
    ...
    const ndb = new loki("test.db", { verbose: true });
