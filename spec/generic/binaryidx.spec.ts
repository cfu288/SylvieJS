...
describe("binary indices", function () {
  ...
  describe("index maintained across batch removes", function () {
    it("works", function () {
      const db = new loki("batch-removes");
      const items = db.addCollection("items", { indices: ["b"] });
  ...
  describe("positional lookup using get works", function () {
    it("works", function () {
      ...
      let item, dataPosition;

      item = items.findOne({ name: "tyrfing" });
      items.remove(item);
  ...
  describe("checkIndex works", function () {
    it("works", function () {
      ...
      const coll = db.addCollection("bitest", { indices: ["a"] });
      ...
      coll.checkIndex("a", { randomSampling: true, randomSamplingFactor: 0.5 });
