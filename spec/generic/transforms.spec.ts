...
import { ChainTransform } from "../../src/database/collection/chain-transform";
const loki = Sylvie;
...
describe("transforms", function () {
  ...
  describe("parameterized find with $and/$or", function () {
    it("works", function () {
      ...
      const txand = [
        {
          type: "find",
          value: {
            $and: [{ owner: "[%lktxp]OwnerName" }, { name: "[%lktxp]Name" }],
          },
        },
      ];
      ...
      let resultsor = items.chain(txor, paramsor).data();
      ...
      let resultsand = items.chain(txand, paramsand).data();
