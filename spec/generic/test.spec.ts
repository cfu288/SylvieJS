...
describe("loki", function () {
  ...
  describe("meta validation", function () {
    it("meta set on returned objects", function () {
      ...
      const coll = tdb.addCollection("tc");
    ...
    it("meta not set on returned objects", function () {
      const tdb = new loki("test.db");
      const coll = tdb.addCollection("tc", { disableMeta: true });
  ...
  describe("batches removes work", function () {
    it("works", function () {
      const rrs = db.addCollection("rrs");
      let idx;
      const count = 100;
      let r1, r2, c1, c2;
  ...
  describe("stepDynamicViewPersistence", function () {
    it("works", function testCollections() {
      ...
      // Throw error on purpose on close
      expect(function () {
        db.close(function () {
          return;
        });
