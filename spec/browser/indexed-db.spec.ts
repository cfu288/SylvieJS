...
import { IndexedDBAdapter } from "../../src/storage-adapter/indexeddb-adapter";
...
describe("IndexedDBAdapter", function () {
  ...
  it("initializes Sylvie/Loki properly", function () {
    ...
    expect(db.persistenceAdapter).toBe(adapter);
  });
  ...
  it("passing 'closeAfterSave' = true option to constructor should close the database after saveDatabase()", function (done) {
    const TEST_DB_NAME = `test_${self.crypto.randomUUID()}.db`;
    const adapter = new IndexedDBAdapter({
      closeAfterSave: true,
    });
  ...
  it("passing 'closeAfterSave' = false option to constructor should close the database after saveDatabase()", function (done) {
    const TEST_DB_NAME = `test_${self.crypto.randomUUID()}.db`;
    const adapter = new IndexedDBAdapter();
    const db = new Sylvie(TEST_DB_NAME, {
      adapter,
  ...
  it("passing 'closeAfterSave' = true option to constructor should close the database after saveDatabaseAsync()", async function () {
    const TEST_DB_NAME = `test_${self.crypto.randomUUID()}.db`;
    const adapter = new IndexedDBAdapter({
      closeAfterSave: true,
    });
  ...
  it("passing 'closeAfterSave' = false option to constructor should close the database after saveDatabaseAsync()", async function () {
    const TEST_DB_NAME = `test_${self.crypto.randomUUID()}.db`;
    const adapter = new IndexedDBAdapter();
    const db = new Sylvie(TEST_DB_NAME, {
      adapter,
