...
import loki from "../dist/sylviejs.js";
...
function profile1a() {
  ...
  rate = rate.toFixed(2);
  console.log(
    "'a' unfiltered, 'b' unindexed (minimal sort function) : " +
      totalMS +
      "ms (" +
      rate +
      " ops/s) " +
...
function profile3a() {
  ...
  rate = rate.toFixed(2);
  console.log(
    "'a' filtered, 'b' unindexed (minimal sort function) : " +
      totalMS +
      "ms (" +
      rate +
      " ops/s) " +
...
function profile3b() {
  ...
  for (idx = 0; idx < loopIterations; idx++) {
    ...
    results = coll
      .chain()
      .find({ a: compare })
      .simplesort("b", { useJavascriptSorting: true })
      .data();
  ...
  rate = rate.toFixed(2);
  console.log(
    "'a' filtered, 'b' unindexed (useJavascriptSorting) : " +
      totalMS +
      "ms (" +
      rate +
      " ops/s) " +
...
function profile4() {
  ...
  for (idx = 0; idx < loopIterations; idx++) {
    ...
    results = coll
      .chain()
      .find({ a: compare })
      .simplesort("b", { disableIndexIntersect: true })
      .data();
  ...
  rate = rate.toFixed(2);
  console.log(
    "'a' filtered, 'b' indexed (old simplesort) : " +
      totalMS +
      "ms (" +
      rate +
      " ops/s) " +
...
function profile5() {
  ...
  for (idx = 0; idx < loopIterations; idx++) {
    ...
    results = coll
      .chain()
      .find({ a: compare })
      .simplesort("b", { forceIndexIntersect: true })
      .data();
...
function profile7() {
  ...
  rate = rate.toFixed(2);
  console.log(
    "'a' filtered, 'b' indexed (smart w/js fallback): " +
      totalMS +
      "ms (" +
      rate +
      " ops/s) " +
...
console.log("");
console.log(
  "average expected resultset size to be sorted: " +
    DOCUMENT_COUNT / QUERY_INDEX_RANGE
);
console.log(
  "expected filtering ratio : " +
    Math.floor(100 - (1 / QUERY_INDEX_RANGE) * 100) +
    "%"
);
console.log("");
