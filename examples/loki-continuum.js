...
var LokiContinuum = (function () {
  ...
  const BalanceTypeEnum = Object.freeze({
    HistoricalBalance: 1,
    VolatileBalance: 2,
    VolatileTransaction: 3,
    InceptionBalance: 4,
  });
  const FundClassEnum = Object.freeze({
    Spending: 1,
    Debt: 2,
    DebtEquity: 3,
    Savings: 4,
  });
  const ActorPeriodicityEnum = Object.freeze({
    Daily: 1,
    Weekly: 2,
    Monthly: 3,
    Yearly: 4,
    ...
    this.activities.addTransform("DoesAffectFund", [
      {
        ...
        value: {
          $or: [
            {
              activityType: ActivityTypeEnum.FundReconcile,
              affectingFund: "[%lktxp]fundId",
            },
            {
              activityType: ActivityTypeEnum.Actor,
              $or: [
                { "affectingActor.fundPrimary": "[%lktxp]fundId" },
                { "affectingActor.fundSecondary": "[%lktxp]fundId" },
              ],
            },
          ],
        },
  ...
  Projector.prototype.activitiesByFund = function (fundId) {
    var result = this.activities
      .chain("DoesAffectFund", { fundId: fundId })
      .data();
  ...
  Projector.prototype.renderUniverse = function (start, end) {
    ...
    var sortedActivities = this.activities
      .chain()
      .simplesort("activityDate")
      .data();
  ...
  function Fund(options) {
    ...
    // volatile, not saved
    this.interimBalance = new Balance(
      this.inceptionBalance,
      this.inceptionDate,
      "inception",
      null,
    );
  ...
  Fund.prototype.postVolatile = function (
    amount,
    postDate,
    name,
    affectingActor,
    ...
    if (
      this.fundClass == FundClassEnum.Debt ||
      this.fundClass == FundClassEnum.DebtEquity
    ) {
    ...
    var bal = new Balance(
      this.interimBalance.amount + trans.amount,
      postDate,
      name,
      affectingActor,
    );
  ...
  Actor.prototype.synchronize = function (syncDate) {
    ...
    while (this.nextTriggerDate <= syncDate) {
      ...
      if (
        this.triggerDateEnd != MAX_DATE &&
        this.triggerDateEnd < this.nextTriggerDate
      ) {
        this.nextTriggerDate = MAX_DATE;
      }
    }
  ...
  Actor.prototype.projectActivities = function (startDate, endDate) {
    ...
    while (this.nextTriggerDate <= endDate) {
      ...
      activities.push(
        new Activity(this.nextTriggerDate, ActivityTypeEnum.Actor, null, this),
      );
  ...
  Actor.prototype.executeActivity = function (activity) {
    ...
    var fpr = singletonContinuum.db
      .getCollection("funds")
      .get(this.fundPrimary);
    var fsr = null;

    if (this.fundSecondary)
      fsr = singletonContinuum.db
        .getCollection("funds")
        .get(this.fundSecondary);

    if (
      fpr.fundClass == FundClassEnum.Debt ||
      fpr.FundClass == FundClassEnum.DebtEquity
    ) {
    ...
    if (this.isLoanPayment) {
      ...
      activity.renderedBalancePrimary = fpr.postVolatile(
        principlePaid,
      ...
      if (fsr != null) {
        ...
        fsr.postVolatile(
          -(this.triggerAmount - principlePaid),
          activity.activityDate,
          this.name + " (Overhead)",
          this.$loki,
        );
        activity.renderedBalanceSecondary = fsr.postVolatile(
          -principlePaid,
          activity.activityDate,
          this.name + " (Principle)",
          this.$loki,
    ...
    if (this.isCompoundInterestAccrual) {
      if (this.periodicity != ActorPeriodicityEnum.Monthly) {
        console.log(
          "An interest accrual actor " +
            this.name +
            " is not set to monthly periodicity!",
        );
      } else {
        var calculatedRate =
          ((this.periodicityUnits / 12.0) * this.compoundInterestRate) / 100.0;
...
function runProjection() {
  ...
  funds.find().forEach(function (fund) {
    if (fund.volatileBalances.length > 0) {
      ...
      console.log(
        "  Final Balance : " +
          fund.volatileBalances[fund.volatileBalances.length - 1].amount,
      );
      console.log(
        "  Volatile Transaction Count : " + fund.volatileTransactions.length,
      );
    } else {
      console.log(
        "Fund (" +
          fund.name +
          ") No Activity, Inception Balance : " +
          fund.inceptionBalance,
      );
    }
  });
...
function dbLoaderCallback() {
  ...
  setTimeout(function () {
    console.log();
    var checkingFund = continuum.db
      .getCollection("funds")
      .findOne({ name: "checking" });
    ...
    console.log(
      "wait 5 seconds before quitting to increase checking by 10 on next run",
    );
  }, 500);
}
  ...
  activities.forEach(function (obj) {
    fsdesc = obj.affectingActor.fundSecondary
      ? " | balance ( " +
        continuum.db
          .getCollection("funds")
          .get(obj.affectingActor.fundSecondary).name +
        ") : " +
        obj.renderedBalanceSecondary.amount
      : "";

    console.log(
      "description : " +
        obj.affectingActor.name +
        " | amount : " +
        obj.affectingActor.triggerAmount +
        " | balance ( " +
        continuum.db.getCollection("funds").get(obj.affectingActor.fundPrimary)
          .name +
        ") : " +
        obj.renderedBalancePrimary.amount +
        fsdesc,
    );
  });
}
